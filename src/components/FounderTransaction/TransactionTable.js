...
import { FaSortUp, FaSortDown, FaSort, FaDownload, FaEye, FaFilter, FaStripe, FaSearch } from 'react-icons/fa';
import { transactionService } from 'src/services/transactionService';
import { useRouter } from 'next/router';
import InvestmentDetailsModal from '../FounderInvestment/InvestmentDetailsModal';
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs uppercase bg-gradient-to-r from-blue-700 to-blue-500 text-white sticky top-0">
              <tr>
                <th className="px-6 py-4 cursor-pointer hover:bg-blue-600 transition-colors" onClick={() => handleSort('id')}>
                  <div className="flex items-center">
                ...
                transactions.map((transaction, index) => (
                  <tr
                    ...
                    className={`border-b hover:bg-blue-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                      } ${hoveredRow === transaction.id ? 'bg-blue-50' : ''}`}
                    onMouseEnter={() => setHoveredRow(transaction.id)}
                    onMouseLeave={() => setHoveredRow(null)}
                  >
                    <td className="px-6 py-4 font-semibold text-gray-700">
                      <span className="bg-gray-100 text-gray-800 py-1 px-2 rounded-md">
                        #{transaction.id}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-semibold text-blue-700">
                      <span 
                        className="bg-blue-100 text-blue-800 py-1 px-2 rounded-md hover:bg-blue-200 transition-colors"
                      >
                        #{transaction.investmentId}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <button
                          onClick={() => navigateToInvestmentDetails(transaction.investmentId)}
